...
function getAuthorCode(type) {
    return new Promise(async resolve => {
        $.get({ url: `https://raw.githubusercontent.com/sngxpro/QuanX/master/share/sngxpro.json` }, (code, message, data) => {
            try {
                if (code) {
...
function readShareCode() {
  console.log(`小小少年跑起来啦`)
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/sngxpro/QuanX/master/share/sharecode.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
