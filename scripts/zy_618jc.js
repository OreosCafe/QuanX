...
async function zy(){
let usernames = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
console.log(`\n*********开始${usernames}助力模块***********\n`);
...
async function getlist(){
 let day = new Date().getDate()+1
 const MyRequest = GetRequest(`index/brandRank?quizId=${day}&t=${new Date().getTime()}`)
 return new Promise((resolve) => {
    $.get(MyRequest,async(error, response, data) =>{
    try{
        const result = JSON.parse(data)
        ...
        if(result && result.code && result.code == 200){
       //activityid = day
       console.log("查看今日销量排行\n")
      for(let i = 0; i < 10; i++){
       brandlistArr.push(result.data[i].id)
